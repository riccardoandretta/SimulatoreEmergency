...
public class Simulatore {
	...
	private void processEvent(Event e) {
		...
		case TRIAGE: // gli assegno uno stato
			...
			// ho assegnato il codice, so che sei in attesa, ma non so dire quanto dovrà
			// aspettare (non riesco a schedulare l'evento)
			// quello che posso fare ora è prevedere il timeout
		...
		case USCITA:
			...
			pazientiCurati++;
			studiOccupati--; // studio libero --> chiamo paziente nuovo

			// Decidere chi deve essere chiamato (sulla base di colore e ora di arrivo) tra
			// i pazienti in attesa
			Paziente paz = attesa.peek();
			if (paz != null) {
				// lo chiamo // Schedulare per ADESSO la CHIAMATA del paziente
				queue.add(new Event(e.getOra(), EventType.CHIAMATA, paz));
			}
			break;

		case TIMEOUT_WHITE:
			if (e.getPaziente().getStato() == StatoPaziente.WHITE) { // perché potrei avere uno stato diverso
				attesa.remove(e.getPaziente());
				pazientiAbbandonati++;
				e.getPaziente().setStato(StatoPaziente.OUT);
			}
		...
		case TIMEOUT_YELLOW:
			...
			break;
		case TIMEOUT_RED:
			if (e.getPaziente().getStato() == StatoPaziente.RED) { // perché potrei avere uno stato diverso
				attesa.remove(e.getPaziente());
				pazientiMorti++;
		...
		case POLLING:
			if(studiOccupati < NS && !attesa.isEmpty()) {
